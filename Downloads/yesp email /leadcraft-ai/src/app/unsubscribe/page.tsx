...
async function doUnsubscribe(token: string): Promise<"ok" | "notfound" | "already"> {
...
export default async function UnsubscribePage({
  ...
  if (!token) {
    return (
      <Shell>
        ...
        <p className="text-sm text-slate-500">This unsubscribe link is missing a token. Please use the link from the email you received.</p>
      </Shell>
    );
  }
    ...
    if (result === "notfound") {
      return (
        <Shell>
          ...
          <p className="text-sm text-slate-500">We could not find your subscription. The link may be expired or already used.</p>
        </Shell>
      );
    }
      ...
      <Shell>
        ...
        <p className="text-sm text-slate-500">
          {result === "already"
            ...
            : "You've been successfully removed from this outreach list. You will no longer receive emails from this campaign."}
        </p>
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      ...
      <Link
        href={`/unsubscribe?token=${token}&confirm=1`}
