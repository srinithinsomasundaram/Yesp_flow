...
export default async function ActivityPage({
  ...
  const { page: pageStr = "0" } = await searchParams;
  const page   = Math.max(0, parseInt(pageStr, 10) || 0);
  const offset = page * ACTIVITY_PAGE_SIZE;

  const logs = await getEmailActivity(ACTIVITY_PAGE_SIZE, offset);
  ...
  const hasNext = logs.length === ACTIVITY_PAGE_SIZE;
                      ...
                      {entries.map((log) => {
                        ...
                        return (
                          <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-5 py-3">
                              <StatusBadge status={log.resendStatus} />
                            </td>
