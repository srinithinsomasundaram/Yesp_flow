...
export async function getCampaigns() {
  ...
  const campaigns = (data || []).map((camp) => ({
    ...camp,
    steps: (camp.steps || []).sort((a: any, b: any) => a.stepNumber - b.stepNumber),
  }));
...
export async function getCampaign(id: string) {
  ...
  if (error || !data) {
    dbLog("getCampaign", error);
    return null;
  }

  // Fetch linked email account separately to avoid FK join issues
...
export async function createCampaign(data: {
  ...
  if (campError || !campaign) {
    dbLog("createCampaign", campError);
    return { success: false, error: campError };
  }
  ...
  if (data.steps && data.steps.length > 0) {
    ...
    if (stepsError) {
      dbLog("createCampaignSteps", stepsError);
    }
  }

  // Auto-assign all existing contacts
  const { data: contacts } = await supabase.from("Contact").select("id");
  if (contacts && contacts.length > 0) {
    const statesToInsert = contacts.map((c) => ({
      contactId: c.id,
      campaignId: campaign.id,
      currentStep: 0,
      status: "New",
    ...
    await supabase.from("ContactCampaignState").insert(statesToInsert);
  }
...
export async function updateCampaign(
  ...
  const { error } = await supabase.from("Campaign").update(updates).eq("id", id);
  revalidatePath("/campaigns");
